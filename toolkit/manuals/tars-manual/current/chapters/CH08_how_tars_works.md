...
## Builder stack (baked UX)
The “builder stack” is a set of instruction modules that makes the UX consistent across chats.

Example: when you say you want to upload files/folders to the repo, the router should trigger the **HUB_IMPORT Upload Wizard** (dest root + mode → job JSON → execute + verify).

This matters because it turns a fragile ad-hoc process into a repeatable workflow.
