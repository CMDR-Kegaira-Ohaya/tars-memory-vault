...
### Home
Shell overview.
Shows current mount, recent saves, and quick actions.
...
### Runs
The active viewport.
Mounted cartridges are rendered or run here.

### Saves
Save-slot management.
All persistence routes through this mode.

### System
Diagnostics, cartridge metadata, shell state, renderer or engine info, settings.
...
- eject and resume are first-class operations
...
### Build target C — cartridge bay
Support:
- mount from repo
- mount from disk
- mount from drag-and-drop

### Build target D — runs viewport
Create the main render/run viewport.

### Build target E — save manager
Route all writes only into `terminal/saves/<tag>/`.

### Build target F — manifest system
